...
function getResourceIdAttributes( dbModel, isPrimaryKey, isForeignKey, isSecondaryKey ) {
  ...
  Object.keys( dbModel.listAttributes ).forEach( function( key ) {
    debug( 'getResourceIdAttributes:', dbModel.model, key, 'isPrimaryKey:', isPrimaryKey, b( dbModel.listAttributes[key].is_primary_key ), 'isForeignKey:', isForeignKey, b( dbModel.listAttributes[key].is_foreign_key ), 'isSecondaryKey:', isSecondaryKey, b( dbModel.listAttributes[key].is_secondary_key ) );
    debug( 'getting the model', dbModel );
    if( dbModel.listAttributes[key] && isPrimaryKey === b( dbModel.listAttributes[key].is_primary_key )
