...
function generateResourceType(model, pathType, relativeParentModel) {
  try {
    if (!model.expose.collection) {
      debug('ignoring collection for resource', model.model);
    }
  } catch(err) {
    debug('model.expose.collection not found');
    return generatePathFromModel(model, pathType, relativeParentModel);
  }
  return;
}
  ...
  dbModel.models.forEach( function( model ) {
    if( model.isPublic === undefined || model.isPublic == true ) {
      findDescendants( model.model, dbModel.models2, true ).forEach( function( descendantModel ) {
        var collectionResource = generateResourceType(dbModel.models2[descendantModel], 'collection', model.model);
        if (collectionResource) { swaggersPaths.push(collectionResource) }
        var entityResource = generateResourceType(dbModel.models2[descendantModel], 'entity', model.model);
        if (entityResource) { swaggersPaths.push(entityResource) }
      });
    }
  } );
...
function flattenParametersArray( _swaggerDoc ) {
  Object.keys( _swaggerDoc.getPaths.paths ).forEach( function( path ) {
    //debug(path)
    Object.keys( _swaggerDoc.getPaths.paths[ path ] ).forEach( function( verb ) {
      _swaggerDoc.getPaths.paths[ path ][ verb ].parameters = [].concat.apply([], _swaggerDoc.getPaths.paths[ path ][ verb ].parameters);
      //debug(_swaggerDoc.getPaths.paths[ path ][ verb ].parameters)
    })
  })
  return _swaggerDoc;
}
